...
export default function IdentificationForm(props) {
  ...
  function getHealthZones() {
    ...
    if (props.provinceName) {
      Object.keys(data.provinces[props.provinceName].health_zones).forEach(
        function (healthZone) {
  ...
  function getHealthAreas() {
    ...
    if (props.provinceName && props.healthZoneName) {
      Object.keys(
        data.provinces[props.provinceName].health_zones[props.healthZoneName]
          .health_areas
      ).forEach(function (healthArea) {
  ...
  function getVillages() {
    ...
    if (props.provinceName && props.healthZoneName && props.healthAreaName) {
      Object.keys(
        data.provinces[props.provinceName].health_zones[props.healthZoneName]
          .health_areas[props.healthAreaName].villages
      ...
      <View style={styles.inputContainer}>
        <View style={{ flexDirection: "row" }}>
          ...
          <TextInput
            ...
            editable={!props.validate}
            onChange={(e) => props.setRegisteredNurse(e.nativeEvent.text)}
            defaultValue={props.registeredNurse}
            placeholder="Nom de l'infirmier titulaire"
          />
        </View>
          ...
          <RNPickerSelect
            ...
            onValueChange={(value) => {
              ...
              value &&
                props.setHealthAreaName(
                  Object.keys(
                    data.provinces[props.provinceName].health_zones[value]
                      .health_areas
                  )[0]
                );
              ...
              // set health zone id
              value &&
                props.setHealthZoneId(
                  data.provinces[props.provinceName].health_zones[value].id
                );
            }}
              ...
              value &&
                props.setVillageName(
                  Object.keys(
                    ...
                    ].health_areas[value].villages
                  )[0]
                );

              // set health area id
              value &&
                props.setHealthAreaId(
                  data.provinces[props.provinceName].health_zones[
                    props.healthZoneName
                  ].health_areas[value].id
                );
            }}
